...
function calculateSimilarity(answer1, answer2) {
  ...
  if (norm1.includes(norm2) || norm2.includes(norm1)) return 70
  return 0
}
...
async function shouldFilterProfile(currentUserId, candidateId) {
  try {
    ...
    const dealBreakerMatch =
      currentUserDealBreakersList.some(db => candidateRedFlagsList.includes(db)) ||
...
async function calculateBehaviorScore(userId, candidateId) {
  try {
    ...
    if (likedProfiles.length > 0) {
      ...
      if (locationMatches / likedProfiles.length > 0.5) score += 10
    }
  ...
  try {
    ...
    const scoredCandidates = candidates.map(candidate => {
      ...
      if (user.age && candidate.age) {
        ...
        else if (ageDiff <= 15) score += 5 // Small market boost for expanded age range
      }
