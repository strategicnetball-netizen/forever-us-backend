...
router.get('/activity-dashboard', authenticate, async (req, res) => {
    try {
        ...
        // Potential dates (mutual matches with conversations)
        const matches = await prisma.like.findMany({
            where: {
                userId,
                likedBy: { some: { userId: { not: userId } } }
            },
        ...
        const potentialDates = await prisma.message.count({
            where: {
                OR: [
                    { senderId: userId, recipientId: { in: matches.map(m => m.likedUserId) } },
                    { recipientId: userId, senderId: { in: matches.map(m => m.likedUserId) } }
                ]
            }
        });
        // Days active
