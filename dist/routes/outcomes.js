...
router.get('/insights', authenticate, async (req, res, next) => {
    try {
        ...
        const successfulMatches = matches.filter(m => ['first_date', 'second_date', 'relationship'].includes(m.dateOutcome));
        const conversationStats = await Promise.all(successfulMatches.map(async (match) => {
...
router.get('/patterns', authenticate, async (req, res, next) => {
    try {
        ...
        const patterns = {
            ...
            successRate: Math.round((allMatches.filter(m => ['first_date', 'second_date', 'relationship'].includes(m.dateOutcome)).length / allMatches.length) * 100)
        };
        res.json(patterns);
    }
    catch (err) {
